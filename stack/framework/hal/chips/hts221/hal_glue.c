...
uint8_t Sensor_IO_Write(void *handle, uint8_t WriteAddr, uint8_t *pBuffer, uint16_t nBytesToWrite) {
  ...
  if(i2c_write_memory(handle, HTS221_I2C_ADDRESS, WriteAddr, 8, pBuffer, nBytesToWrite))
    return 0;
  else
    return 1;
}
...
uint8_t Sensor_IO_Read(void *handle, uint8_t ReadAddr, uint8_t *pBuffer, uint16_t nBytesToRead) {
  ...
  if(i2c_read_memory(handle, HTS221_I2C_ADDRESS, ReadAddr, 8, pBuffer, nBytesToRead))
    return 0;
  else
    return 1;
}
